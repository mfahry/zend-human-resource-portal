...
class MTask{
    ...
    public function selectDB_taskReport($userid = NULL, $project = NULL, $month = NULL, $year = NULL, $task_id = NULL){
		$data = array();
        
		$query = "SELECT DISTINCT(t.task), u.user_id, u.photo, 
				MIN(ts.update_dtm) AS initial_dtm, ts.status, t.task_id,
				t.target, t.priority_id, u.name
				FROM task t JOIN user u ON t.user_id=u.user_id
				JOIN task_status ts ON ts.task_id = t.task_id ";
		
		//$query .= "	WHERE t.user_id = '75080710' ";
		
		if($userid != NULL && $month != NULL && $year != NULL && $project!= ''){
			$query .= " WHERE t.user_id = '".$userid."'  
						AND MONTH(ts.update_dtm) = '".($month<10? "0$month":$month)."' 
						AND YEAR(ts.update_dtm) = '".$year."'
						AND t.task LIKE '%[".$project."]%'";
		}elseif($userid != NULL && $month != NULL && $year != NULL && $project== ''){
			$query .= " WHERE t.user_id = '".$userid."'  
						AND MONTH(ts.update_dtm) = '".($month<10? "0$month":$month)."' 
						AND YEAR(ts.update_dtm) = '".$year."'";	
		...
		}elseif($userid == '' && $month != NULL && $year != NULL && $project!= ''){
			$query .= " AND MONTH(ts.update_dtm) = '".($month<10? "0$month":$month)."' 
						AND YEAR(ts.update_dtm) = '".$year."'
						AND t.task LIKE '%[".$project."]%'";
		}elseif($userid == '' && $month != NULL && $year != NULL && $project== ''){
			$query .= " AND MONTH(ts.update_dtm) = '".($month<10? "0$month":$month)."' 
			...
			$query .= " WHERE MONTH(ts.update_dtm) = '".($month<10? "0$month":$month)."'
						AND t.task LIKE '%[".$project."]%'";
		}elseif($userid == '' && $month != NULL && $year == NULL && $project== ''){
			$query .= " WHERE MONTH(ts.update_dtm) = '".($month<10? "0$month":$month)."'";
						
		}elseif($userid == NULL && $_SESSION['position_id'] != 1 && $month != NULL && $year != NULL && $project!= ''){
		...
		$query .= "	GROUP BY ts.task_id 
				ORDER BY ts.task_id DESC,ts.update_dtm DESC";
	...
	public function selectDB_taskTop5($userid = NULL, $procent = NULL, $month = NULL, $year = NULL, $task_id = NULL){
        ...
        if($month != NULL && $userid != '-1') 
			$query .= " AND MONTH(ts.update_dtm) = '".$month."'";
		elseif($month != NULL && $userid == '-1') 
	...
	function get_taskByCalendar($userid = NULL, $month = NULL, $year = NULL, $project = NULL){
		$data = array();
        $query = "SELECT DISTINCT(t.task), u.user_id, u.photo, ts.update_dtm, ts.status, t.task_id, t.priority_id, t.target, u.name
                    FROM task t JOIN user u ON t.user_id=u.user_id
					JOIN task_status ts ON ts.task_id = t.task_id";
					
        if($userid != NULL && $userid != '' && $month != NULL && $year != NULL && $project!=''){
			$query .= " WHERE t.user_id = '".$userid."'  
						AND MONTH(ts.update_dtm) = '".$month."' 
						AND YEAR(ts.update_dtm) = '".$year."'
						AND t.task LIKE '%[".$project."]%'";
		}elseif($userid != NULL && $userid != '' && $month != NULL && $year != NULL && $project==''){
			$query .= " WHERE t.user_id = '".$userid."'  
						AND MONTH(ts.update_dtm) = '".$month."' 
						AND YEAR(ts.update_dtm) = '".$year."'";	
    ...
    public function insertDB_task($task_id,$userid, $task, $priority,$target){
			$query = "INSERT INTO task(task_id,user_id, task, priority_id,target) 
						VALUES ('".$task_id."','".$userid."', '".$task."', '1','".$target."')";			
