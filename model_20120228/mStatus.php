...
class MStatus{
	...
	public function selectDB_homeUnion($userid = NULL, $limitto = NULL, $limitfrom = NULL, $newDate = NULL){
		...
		$query .= " ORDER BY tanggal DESC)
				  	UNION
				  (SELECT t.task_id AS id, ts.update_by AS userid, u.name, 'title' AS title, 
				  	...
				  	FROM task t JOIN task_status ts ON ts.task_id = t.task_id
				   	JOIN user u ON ts.update_by = u.user_id WHERE ts.is_show = 1";
					//if($newDate != NULL) $query .= " AND ts.update_dtm > '".$newDate."'";
					//if($userid != NULL) $query .= " AND ts.update_by = '".$userid."'";
		$query .= " ORDER BY tanggal DESC)
				  	UNION
