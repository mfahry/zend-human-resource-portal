...
class MTask{
    ...
    public function selectDB_task($userid = NULL, $procent = NULL, $month = NULL, $year = NULL, $task_id = NULL){
		...
		$query .= " ORDER BY b.priority_id ASC, ts.update_dtm desc";
	...
	function get_taskByCalendar($userid = NULL, $month = NULL, $year = NULL){
		$data = array();
        $query = "SELECT DISTINCT(t.task), u.user_id, u.photo, ts.update_dtm, ts.status, t.task_id, u.name
                    FROM task t JOIN user u ON t.user_id=u.user_id
					JOIN task_status ts ON ts.task_id = t.task_id";
    ...
    public function insertDB_task($taskid, $userid, $task, $priority){
		$query = "INSERT INTO task (task_id,user_id, task, priority_id) VALUES ($taskid,'".$userid."', '".$task."', '1')";			

		if(!$this->registry->db->execute($query)) throw new Exception('Gagal menyimpan data task');
    }
	
	public function insertDB_userTask($task,$userid){
		$query = "INSERT INTO user_task (task_id,user_id) VALUES ('".$task."', '".$userid."')";			
