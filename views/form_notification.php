...
$task		= $this->registry->mTask->get_taskNotif($_SESSION['userid']);
 
if((count($dataRec0)>0)||(count($dataRec2)>0)||(count($dataRec3)>=2) || (count($dataLembur) > 0) || (count($notification) > 0) || (count($task) > 0)){
?>
...
if(count($task) > 0){ ?>
    <li> 
        ...
        <a id="notifComment" href="#" onClick="return false">
    	Anda mendapat task baru (<?php echo count($task); ?>)</a>
        
      <div id="commentContainer" class="notifContainer">
        <ol>
          <?php 
		  foreach($task as $komen){ 
			$tsk = $this->registry->mTask->get_taskTitle($komen->TASK_ID);
			foreach($tsk as $task){
			  ?>
			  <li><a href="index.php?mod=attendance&userid=<?=$_SESSION['userid']?>"><?php echo $task->TASK.' ('.$task->TARGET.')'; ?></a></li>
			  <?php
			}
		  } 
		  ?>
